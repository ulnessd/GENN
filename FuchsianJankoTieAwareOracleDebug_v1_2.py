#!/usr/bin/env python3
"""
FuchsianJankoTieAwareOracleDebug_v1_2.py

Tie-aware oracle-debug program for the Janko Hurwitz GINN experiments.
...
PROGRAM = "FuchsianJankoTieAwareOracleDebug_v1_2.py"
VERSION = "1.2"
...
def run_contains_matching_surface(run_root: Path, group_name: Optional[str], triple_filter: Optional[set[int]] = None) -> bool:
    ...
    for p in surface_dir.glob("*.json"):
        ...
        if group_name is not None:
            sg = str(surf.get("finite_group_triple", {}).get("group_name", surf.get("group", surf.get("finite_quotient", "")))).upper()
            sid = str(surf.get("surface_id", "")).upper()
            if group_name.upper() not in sg and group_name.upper() not in sid:
                continue
        tridx = int(surf.get("finite_group_triple", {}).get("triple_index", 0))
...
def latest_matching_run_under(root: Path, group_name: Optional[str], triple_filter: Optional[set[int]] = None) -> Optional[Path]:
    """Return the newest run under root that actually contains matching Janko surfaces/checkpoints."""
    if not root.exists():
        return None
    runs = [p for p in root.iterdir() if p.is_dir() and p.name.startswith("run_")]
    runs.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    for run in runs:
        if run_contains_matching_surface(run, group_name, triple_filter):
            return run
    return None



def find_surface_files(trained_run_root: Path, group_name: Optional[str], triple_filter: Optional[set[int]]) -> List[Path]:
    ...
    for p in sorted(surface_dir.glob("*.json")):
        try:
            ...
            continue
        if group_name is not None:
            sg = str(s.get("finite_group_triple", {}).get("group_name", s.get("group", s.get("finite_quotient", "")))).upper()
            sid_upper = str(s.get("surface_id", "")).upper()
            if group_name.upper() not in sg and group_name.upper() not in sid_upper:
                continue
        tridx = int(s.get("finite_group_triple", {}).get("triple_index", 0))
        if triple_filter is not None and tridx not in triple_filter:
            continue
...
def build_all_surface_cached(
    ...
    tr = selected_surface.get("finite_group_triple") or janko.build_janko_triple_record(group)
    build_args = SimpleNamespace(
        ...
        kernel_scan_progress_every=int(getattr(args, "kernel_progress_every", 50000)),
        kernel_progress_every=int(getattr(args, "kernel_progress_every", 50000)),
        tile_progress_every=int(args.tile_progress_every),
    )
    all_surface, all_audit = janko.build_schreier_kernel_surface_janko(group, tr, build_args, run_root.name, hurwitz)
...
def summarize_surface_audit(
    ...
    row: Dict[str, Any] = {
        ...
        "group": selected_surface.get("finite_group_triple", {}).get("group_name", selected_surface.get("group", "")),
...
def write_report(run_root: Path, args: argparse.Namespace, summary_rows: List[Dict[str, Any]]) -> None:
    lines: List[str] = []
    lines.append(f"# Janko Tie-Aware Oracle Debug v{VERSION} Report")
    lines.append("")
    lines.append(f"Created: {datetime.now().isoformat(timespec='seconds')}")
    lines.append("")
    lines.append("## Purpose")
    lines.append("")
    lines.append("This debug run compares a selected-generator Janko GINN against both its selected finite atlas and a streamed complete all-Schreier-generator depth-2 oracle, while separating exact word-key misses from distance-level ties.")
    lines.append("")
    ...
    lines.append("")
    for k in ["trained_run_root", "group", "surface", "audit_pairs", "depth", "oracle_top_k", "model_top_k", "engine", "candidate_chunk_size", "pair_batch_size", "virtual_topk_buffer", "hidden_epsilon", "tie_epsilon"]:
    ...
    (run_root / "report" / "janko_tie_aware_oracle_debug_report.md").write_text("\n".join(lines), encoding="utf-8")

def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Tie-aware debug audit of selected Janko Hurwitz GINN predictions against complete all-generator depth-2 oracle.")
    ap.add_argument("--trained-run-root", type=str, default="", help="Path to a completed FuchsianJankoHurwitzTrainer run. If omitted, use latest under --trained-outroot.")
    ap.add_argument("--trained-outroot", type=str, default="janko_hurwitz_training_runs", help="Root used to auto-find latest matching Janko run when --trained-run-root is omitted.")
    ap.add_argument("--group", choices=["J1", "J2"], default="J1", help="Janko group to audit.")
    ...
    ap.add_argument("--outroot", type=str, default="janko_tie_debug_runs")
    ...
    ap.add_argument("--janko-trainer-script", type=str, default=default_local("FuchsianJankoHurwitzTrainer_v1_1.py"))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not args.label:
        args.label = f"{args.group}_oracle_audit_pairs{args.audit_pairs}"
    ...
    if args.trained_run_root:
        ...
        trained_root = latest_matching_run_under(Path(args.trained_outroot).expanduser(), args.group, triple_filter)
    ...
    print(f"group={args.group} audit_pairs={args.audit_pairs} depth={args.depth} oracle_top_k={args.oracle_top_k} model_top_k={args.model_top_k}")
    print("-" * 78)
    ...
    janko = load_module(args.janko_trainer_script, "janko_trainer_v11")
    hurwitz = load_module(args.hurwitz_script, "hurwitz_v16")
    ginn = load_module(args.ginn_script, "ginn_v24")
    janko.install_leq_depth_word_ball_builder(ginn)

    group = janko.build_group_by_name(args.group)
    group.enumerate_from_xy(verbose=bool(args.verbose))
    surface_files = find_surface_files(trained_root, args.group, triple_filter)
    ...
    for sf in surface_files:
        ...
        t0 = time.perf_counter()
        p, q, sampler_kind = zoo.sample_pairs(ginn, selected_surface, int(args.audit_pairs), int(args.seed) + tridx * 100003 + (1 if args.group.upper() == "J1" else 2) * 1000003)
        ...
        all_surface = build_all_surface_cached(janko, hurwitz, group, selected_surface, run_root, args)
