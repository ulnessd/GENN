#!/usr/bin/env python3
"""
FuchsianPSL2qOracleAudit_v1_1.py

Independent oracle-audit program for the PSL(2,q) Hurwitz GINN experiments.
...
PROGRAM = "FuchsianPSL2qOracleAudit_v1_1.py"
VERSION = "1.1"
...
def summarize_surface_audit(
    ...
    hidden = full_d < selected_d - eps
    coverage = np.asarray([a == b for a, b in zip(selected_winner_keys, full_winner_keys)], dtype=bool)
    model_restricted_top1 = contains_at_k(model_keys, selected_winner_keys, 1)
    model_absolute_top1 = contains_at_k(model_keys, full_winner_keys, 1)
    ...
    row: Dict[str, Any] = {
        ...
        "restricted_model_win_rate": float(np.mean(model_restricted_top1)),
        "full_oracle_coverage_rate": float(np.mean(coverage)),
        "hidden_lift_rate": float(np.mean(hidden)),
        "absolute_model_win_rate": float(np.mean(model_absolute_top1)),
    ...
    for k in topks:
        ...
        row[f"absolute_model_recall_at_{k}"] = float(np.mean(contains_at_k(model_keys, full_winner_keys, kk_m)))
        row[f"selected_oracle_contains_full_at_{k}"] = float(np.mean(contains_at_k(selected_keys, full_winner_keys, kk_s)))
        model_pruned = np.min(model_top_d[:, :kk_m], axis=1).astype(np.float64)
        row[f"mean_model_top{k}_pruned_regret"] = float(np.mean(np.maximum(0.0, model_pruned - full_d)))
        row[f"median_model_top{k}_pruned_regret"] = float(np.median(np.maximum(0.0, model_pruned - full_d)))
    ...
    for i in range(n):
        pair_rows.append({
            ...
            "model_equals_selected_top1": int(model_keys[i][0] == selected_winner_keys[i]),
            "selected_equals_full_top1": int(selected_winner_keys[i] == full_winner_keys[i]),
            "model_equals_full_top1": int(model_keys[i][0] == full_winner_keys[i]),
            "hidden_lift": int(bool(hidden[i])),
            "truncation_regret": float(max(0.0, selected_d[i] - full_d[i])),
            "model_top1_regret": float(max(0.0, model_d1[i] - full_d[i])),
            "model_top5_contains_selected": int(selected_winner_keys[i] in model_keys[i][:min(5, max_model_k)]),
            "model_top5_contains_full": int(full_winner_keys[i] in model_keys[i][:min(5, max_model_k)]),
            "selected_top20_contains_full": int(full_winner_keys[i] in selected_keys[i][:min(20, max_selected_k)]),
            "selected_top5_distances": "|".join(f"{x:.8g}" for x in selected_top_d[i, :min(5, selected_top_d.shape[1])].tolist()),
            "model_top5_indices": "|".join(str(int(x)) for x in model_top_i[i, :min(5, model_top_i.shape[1])].tolist()),
            "model_top5_distances": "|".join(f"{x:.8g}" for x in model_top_d[i, :min(5, model_top_d.shape[1])].tolist()),
            "full_top5_virtual_indices": "|".join(str(int(x)) for x in full_top_i[i, :min(5, full_top_i.shape[1])].tolist()),
            "full_top5_distances": "|".join(f"{x:.8g}" for x in full_top_d[i, :min(5, full_top_d.shape[1])].tolist()),
        })
...
def write_report(run_root: Path, args: argparse.Namespace, summary_rows: List[Dict[str, Any]]) -> None:
    lines: List[str] = []
    lines.append(f"# PSL(2,q) Oracle Audit v{VERSION} Report")
    lines.append("")
    lines.append(f"Created: {datetime.now().isoformat(timespec='seconds')}")
    lines.append("")
    lines.append("## Purpose")
    lines.append("")
    lines.append("This audit compares a trained selected-generator GINN against both the selected finite atlas and a streamed complete-generator depth-2 oracle.")
    lines.append("")
    ...
    lines.append("")
    for k in ["trained_run_root", "q", "surface", "audit_pairs", "depth", "oracle_top_k", "model_top_k", "engine", "candidate_chunk_size", "pair_batch_size", "virtual_topk_buffer", "hidden_epsilon"]:
    ...
    if summary_rows:
        cols = [
            "surface_id", "triple_index", "audit_pairs", "restricted_model_win_rate", "full_oracle_coverage_rate", "hidden_lift_rate", "absolute_model_win_rate",
            "restricted_model_recall_at_5", "absolute_model_recall_at_5", "restricted_model_recall_at_20", "absolute_model_recall_at_20",
            "selected_oracle_contains_full_at_20", "mean_truncation_regret", "median_truncation_regret", "mean_model_top1_regret", "median_model_top1_regret",
        ]
    ...
    lines.append("")
    lines.append("- `restricted_model_win_rate`: model top-1 equals the selected-generator oracle winner.")
    lines.append("- `full_oracle_coverage_rate`: selected-generator oracle winner equals the complete depth-2 oracle winner.")
    lines.append("- `hidden_lift_rate`: the complete depth-2 oracle distance is smaller than the selected oracle distance by more than `hidden_epsilon`.")
    lines.append("- `absolute_model_win_rate`: model top-1 equals the complete depth-2 oracle winner.")
    lines.append("- `truncation_regret`: selected oracle distance minus complete depth-2 oracle distance, clipped below at zero.")
    lines.append("- `model_top1_regret`: model top-1 lift distance minus complete depth-2 oracle distance, clipped below at zero.")
    (run_root / "report").mkdir(parents=True, exist_ok=True)
    (run_root / "report" / "psl2q_oracle_audit_report.md").write_text("\n".join(lines), encoding="utf-8")


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Audit selected PSL(2,q) GINN predictions against complete all-generator depth-2 oracle.")
    ...
    ap.add_argument("--outroot", type=str, default="psl2q_oracle_audit_runs")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for sf in surface_files:
        ...
        print(
            ...
            f"coverage={row['full_oracle_coverage_rate']:.4f} hidden={row['hidden_lift_rate']:.4f} "
            f"absolute_win={row['absolute_model_win_rate']:.4f}",
            flush=True,
        )
    ...
    print(f"[done] report={run_root / 'report' / 'psl2q_oracle_audit_report.md'}")
